...
def generate_key():
    ...
    with open("topsecret.key", "wb") as key_file:
        key_file.write(key)

def load_key():
    root = Tk()
    root.withdraw()
    file_path = filedialog.askopenfilename(title="Seleziona la key precedentemente fornita")
...
def main():
    choice = input("Hai già una key? (Sì/No): ").strip().lower()
    
    if choice in ["si", "sì", "s", "y", "yes"]:
        key = load_key()
    elif choice in ["n", "no"]:
        print(Fore.CYAN + "\nGenerata nella cartella corrente la tua key (topsecret.key). NASCONDILA E CONSERVALA, serve a recuperare le tue password!")
        generate_key()
        key = load_key()
    else:
